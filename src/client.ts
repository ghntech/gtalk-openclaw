...
export class GtalkClient {
  ...
  private readonly logger?: GtalkLogger;
  ...
  private async post<T>(path: string, body: Record<string, unknown>): Promise<T> {
    ...
    let json: GtalkResponse<T>;
    let rawText = "";
    try {
      rawText = await res.text();
      json = JSON.parse(rawText) as GtalkResponse<T>;
    } catch {
  ...
  async sendText(
    channelId: string,
    text: string,
    parseMode: "PLAIN_TEXT" | "MARKDOWN" | "HTML" = "PLAIN_TEXT",
  ): Promise<SendMessageResult> {
    return this.post<SendMessageResult>("/api/gtalk/send-message", {
      channelId,
  ...
  async sendTemplate(
    ...
    parseMode?: "PLAIN_TEXT" | "MARKDOWN" | "HTML",
  ): Promise<SendMessageResult> {
    ...
    if (parseMode) content.parseMode = parseMode;
  ...
  async initiateUpload(params: {
    ...
    return this.post<InitiateUploadResult>("/api/gtalk/initiate-upload", {
  ...
  async uploadToS3(presignedUrl: string, data: Buffer, mimeType: string): Promise<void> {
  ...
  async completeUpload(uploadId: string): Promise<CompleteUploadResult> {
    return this.post<CompleteUploadResult>("/api/gtalk/complete-upload", {
      UploadId: uploadId,
    });
  }
  ...
  async sendByFileId(
    ...
    if (mimeType.startsWith("image/")) {
      ...
      return this.sendFile(
        channelId, fileId,
        detail.FileName, mimeType, parseInt(detail.FileSize, 10),
      );
    }
  }
  ...
  async uploadAndSend(params: {
    ...
    if (fileSize > MAX_FILE_SIZE) {
      throw new Error(
        `File size exceeds 100MB limit (${(fileSize / 1024 / 1024).toFixed(2)}MB)`,
      );
    }
    ...
    const initiated = await this.initiateUpload({
      ...
      fileSize,
      mimeType,
      metadata: (isImage || isVideo) ? mediaMeta as Record<string, unknown> : undefined,
    });
    ...
    if (isImage || isVideo) {
      const { buffer: thumbBuffer, mimeType: thumbMime } = await makeThumbnail(
        isVideo ? filePath : fileBuffer,
        mimeType,
      );
   ...
   */
  async createDirectChannel(oaId: string, userId: string): Promise<string> {
    const result = await this.post<CreateDirectChannelResult>(
  ...
  async sendReceipt(params: SendReceiptParams): Promise<void> {
    const now = Date.now();
    await this.post<Record<string, never>>("/api/gtalk/send-message-receipt", {
      oaId: params.oaId,
      receiptMessage: {
        channelId: params.channelId,
  ...
  async modifyMessage(params: {
    ...
    await this.post<Record<string, never>>("/api/gtalk/modify-message", {
      channelId: params.channelId,
   ...
   */
  async configChannelWebhook(params: ConfigChannelProcessingParams): Promise<void> {
